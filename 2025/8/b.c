...
int main(int argc, char **argv) {
  ...
  for (;;) {
    ...
    if ((last_join_size = dsf_join(dsf, SIZE, current_node->idx1,
                                   current_node->idx2)) == SIZE) {
      break;
    }
  }

  printf("%ld\n", (long)points[current_node->idx1].x *
                      (long)points[current_node->idx2].x);
